...
The following code saves a buffer of 5 gait cycle periods per foot to calculate the velocity required by the motor
...
int sampPeriod = 1;
int actTrigState = 1; // Trigger state for the exoskeleton actuator (trigger occurs when a transition from the previous state to actTrig is detected)
...
int resetIdlePin = 44;  // Not Needed!
int errTrigPin = 46;    // Not Needed!
...
float posOD; // ODrive motor position control

// Error Variables
boolean errTrig; boolean errTrig_prev;
...
void setup() {
  ...
  /* Set Motor to IDLE Mode at Start-Up */
  ...
  // Activate the motor by pushing the experimental trigger pushbutton
  ...
  if(SD.exists("/TEST1102/MAIN_EXP")){
  ...
  if (dataFile) {
    ...
    dataFile.print("motorPos"); dataFile.print(","); 
    //dataFile.print("motorODPos"); dataFile.print(",");
    dataFile.print("posStep"); dataFile.print(",");
    //dataFile.print("motorVel"); dataFile.print(",");
    //dataFile.print("iComm"); dataFile.print(","); 
    //dataFile.print("iMeas"); dataFile.print(",");
    ...
    //dataFile.print("resetIdle"); dataFile.print(","); 
...
void loop() {
  ...
  // Set motor to IDLE Mode if Ball Screw Limit is Reached
  /*
  if( (limitSwitchTop == true) || (limitSwitchBottom == true) ){
    requested_state = ODriveArduino::AXIS_STATE_IDLE;
    Serial << "Axis" << motornum << ": Requesting state " << requested_state << '\n';
    if(!odrive.run_state(motornum, requested_state, false)) return;
    Serial.println("Motor set to IDLE mode.");
  ...
  */
  ...
  /* Trigger Functions */
  heelStrikeDetect();             // Detect if a heelstrike has taken place (comment out if State 1 Onset is not the trigger)
  //phaseDetect234(actTrigState); // Detect if the triggering gait transition has taken place (comment out if heelstrike is the trigger)
  ...
  if((expTrig == false) && (lastExpTrig == true) && (pos > 0)){
    motorCond = 1;
    pos = pos - posStep;
    odrive.TrapezoidalMove(motornum, pos);
  ...
  //else if((hsDetectLeft == true) && (hsDetectRight == false) && (lcLeft <= maxForceLeftLC ) && (lcRight <= maxForceRightLC)){
    motorCond = 3;
    pos = pos + posStep;
    odrive.TrapezoidalMove(motornum, pos);
  }
  ...
  //else if((hsDetectLeft == false) && (hsDetectRight == true)  && (lcLeft <= maxForceLeftLC ) && (lcRight <= maxForceRightLC)){
    motorCond = 4;
    pos = pos - posStep;
    odrive.TrapezoidalMove(motornum, pos);
  }
  ...
  // Record motor values (causes non-trivial delays in MCU speed)
  //odrive_serial.write("r axis1.encoder.pos_estimate\n");
  //posOD = odrive.readFloat();
  ...
  if(expTrig == true){
    ...
    if (dataFile) {
      ...
      //dataFile.print(posOD); dataFile.print(",");
      dataFile.print(posStep); dataFile.print(",");
      //dataFile.print(vel_meas); dataFile.print(",");
      //dataFile.print(iQ_comm); dataFile.print(","); 
      //dataFile.print(iQ_meas); dataFile.print(",");
      //dataFile.print(iRegen); dataFile.print(",");
      //dataFile.print(hsDetectLeft); dataFile.print(","); dataFile.print(hsDetectRight); dataFile.print(",");
      //dataFile.print(limitSwitchTop); dataFile.print(","); dataFile.print(limitSwitchBottom); dataFile.print(",");
      dataFile.print(actTrigLeft); dataFile.print(","); dataFile.print(actTrigRight); dataFile.print(",");
  ...
  // Detect the Errors in the ODrive if Trigger is Activated
  //errTrig = digitalRead(errTrigPin);
  //errorDetect();
  ...
  // Set the previous error trigger pin
  errTrig_prev = errTrig;
